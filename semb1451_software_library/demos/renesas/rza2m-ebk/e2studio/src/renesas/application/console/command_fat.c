...
uint8_t  g_sample_app_rw_buff[FF_VOLUMES][RW_BUFF_SIZE] __attribute__ ((section ("UNCACHED_BSS"), aligned(8)));
...
int16_t FAT_Sample_Type(int_t iArgCount, char_t **ppszArgument, pst_comset_t pCom)
{
    ...
    else
    {
        ...
        for (;;)
        {
            fr = f_read(&g_fil[side], &g_sample_app_rw_buff[side][0], RW_BUFF_SIZE, &br);
            ...
            else
            {
                ...
                {
                    printf("%02X ", g_sample_app_rw_buff[side][cnt]);
...
int16_t FAT_Sample_Write(int_t iArgCount, char_t **ppszArgument, pst_comset_t pCom)
{
    ...
    {
        memcpy(&g_sample_app_rw_buff[side][0], &p_write_buff[0], sizeof(p_write_buff));
        fr = f_write(&g_fil[side], &g_sample_app_rw_buff[side][0], sizeof(p_write_buff), &bw);
        if (FR_OK != fr)
        {
            printf("write error.\r\n");
        }
