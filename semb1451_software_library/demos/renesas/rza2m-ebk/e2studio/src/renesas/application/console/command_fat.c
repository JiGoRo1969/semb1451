...
uint8_t  g_app_rw_buff[FF_VOLUMES][RW_BUFF_SIZE] __attribute__ ((section ("UNCACHED_BSS"), aligned(8)));
FATFS    g_fatfs[FF_VOLUMES] __attribute__ ((section ("UNCACHED_BSS")));
FIL      g_fil[FF_VOLUMES] __attribute__ ((section ("UNCACHED_BSS")));

uint32_t g_fat_sem_access[FF_VOLUMES] = { 0uL };
...
int16_t FAT_Att(int_t iArgCount, char_t **ppszArgument, pst_comset_t pCom)
{
    ...
    memset(&g_fatfs[side], 0, sizeof(FATFS));
    ...
    if (0uL == g_fat_sem_access[side])
    {
        chk_sem = R_OS_SemaphoreCreate(&g_fat_sem_access[side], 1uL);
        if (false == chk_sem)
        {
    ...
    chk = f_mount(&g_fatfs[side], drive_str, 1);  /* Mount logical drive. opt = 1 : Mount immediately */
    if (FR_OK == chk)
    {
        printf("device attach success.\r\n");
        printf("SIDE <%d>\r\n", side);
        if (g_fatfs[side].fs_type < FS_TYPE_NUM)
        {
            printf("\tFilesystem type          = %s\r\n", p_fs_type_str[g_fatfs[side].fs_type]);
        ...
        printf("\tSector size [bytes]      = %d\r\n", FF_MAX_SS);
#endif
        printf("\tCluster size [sectors]   = %d\r\n", g_fatfs[side].csize);   /* Cluster size [sectors]   */
        printf("\tNumber of FATs (1 or 2)  = %d\r\n", g_fatfs[side].n_fats);  /* Number of FATs (1 or 2)  */
        printf("\tSize of an FAT [sectors] = %d\r\n", g_fatfs[side].fsize);   /* Size of an FAT [sectors] */
    }
    else
    {
...
int16_t FAT_Det(int_t iArgCount, char_t **ppszArgument, pst_comset_t pCom)
{
    ...
    R_OS_SemaphoreDelete(&g_fat_sem_access[side]);

    return CMD_OK;
}
/*******************************************************************************
 End of function FAT_Det
 ******************************************************************************/
...
int16_t FAT_Dir(int_t iArgCount, char_t **ppszArgument, pst_comset_t pCom)
{
    ...
    chk_sem = R_OS_SemaphoreWait(&g_fat_sem_access[side], FS_SEM_WAITTIME);
    if (false == chk_sem)
    {
...
#if (FF_USE_LFN != 0)
            if (FS_EXFAT != g_fatfs[side].fs_type)
            {
    #if (FF_LFN_UNICODE == 0) || (FF_LFN_UNICODE == 2)
    ...
    R_OS_SemaphoreRelease(&g_fat_sem_access[side]);

    return CMD_OK;
}
/*******************************************************************************
 End of function FAT_Dir
 ******************************************************************************/
...
int16_t FAT_Type(int_t iArgCount, char_t **ppszArgument, pst_comset_t pCom)
{
    ...
    fr = f_open(&g_fil[side], p_path_name, FA_READ);
    if (FR_OK != fr)
    {
    ...
    else
    {
        ...
        for (;;)
        {
            fr = f_read(&g_fil[side], &g_app_rw_buff[side][0], RW_BUFF_SIZE, &br);
            ...
            else
            {
                ...
                {
                    printf("%02X ", g_app_rw_buff[side][cnt]);
...
int16_t FAT_Write(int_t iArgCount, char_t **ppszArgument, pst_comset_t pCom)
{
    ...
    fr = f_open(&g_fil[side], p_path_name, FA_OPEN_APPEND | FA_WRITE);
    if (FR_OK != fr)
    {
        printf("open file error.\r\n");
    }
    ...
    {
        memcpy(&g_app_rw_buff[side][0], &p_write_buff[0], sizeof(p_write_buff));
        fr = f_write(&g_fil[side], &g_app_rw_buff[side][0], sizeof(p_write_buff), &bw);
        if (FR_OK != fr)
        {
            printf("write error.\r\n");
        }
...
int16_t FAT_Create(int_t iArgCount, char_t **ppszArgument, pst_comset_t pCom)
{
    ...
    fr = f_open(&g_fil[side], p_path_name, FA_CREATE_NEW | FA_WRITE);
    if (FR_OK != fr)
    {
