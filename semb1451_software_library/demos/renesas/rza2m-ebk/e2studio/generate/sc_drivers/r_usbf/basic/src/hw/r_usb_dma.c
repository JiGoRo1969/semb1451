...
usb_dma_int_t   g_usb_cstd_dma_int __attribute__ ((section ("USB_DATA_RAM")));
...
void usb_dma_driver(void)
{
    ...
    /* USB1/USBA */
        ...
        channel = usb_dma_ref_ch_no(ip, g_usb_cstd_dma_int.buf[g_usb_cstd_dma_int.rp].fifo_type);
        pipe = g_usb_cstd_dma_pipe[ip][channel];

#if USB_CFG_CNTMD == USB_CFG_CNTMDOFF
        /* max packet size */
        buffer = hw_usb_read_pipemaxp((usb_utr_t *)NULL);
        buffer = (uint16_t) (buffer & USB_MXPS);
#else  /* USB_CFG_CNTMD == USB_CFG_CNTMDOFF */
        /* buffer size */
    	buffer = usb_cstd_get_buf_size(USB_NULL, pipe);
#endif /* USB_CFG_CNTMD == USB_CFG_CNTMDOFF */

    	size = g_usb_cstd_dma_size[ip][channel];

        if( size != buffer )
        {
            /* bval control for transfer enable fifo 2 usb control */
            hw_usb_set_bval(USB_NULL, g_usb_cstd_dma_int.buf[g_usb_cstd_dma_int.rp].fifo_type);
        }
...
void usb_dma_buf2dxfifo_complete_event_set(uint16_t ip_no, uint16_t use_port)
{
    ...
    p = get_usb_int_buf();
    p->ip           = ip_no;
	p->type         = USB_INT_DXFIFO;
    p->fifo_type    = use_port;
#else /*(BSP_CFG_RTOS_USED == 1)*/
    g_usb_cstd_dma_int.buf[g_usb_cstd_dma_int.wp].ip = ip_no;
    g_usb_cstd_dma_int.buf[g_usb_cstd_dma_int.wp].fifo_type = use_port;
    g_usb_cstd_dma_int.wp = ((g_usb_cstd_dma_int.wp+1) % USB_INT_BUFSIZE);
