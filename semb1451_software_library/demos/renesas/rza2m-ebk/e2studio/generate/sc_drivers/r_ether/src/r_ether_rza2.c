...
#if (ETHER_CFG_USE_MMU)
    /*R_MMU_PAtoVA( papp_tx_desc[channel]->p_buf, pbuf);*/
...
static void ether_init_descriptors (uint32_t channel)
{
    ...
    for (i = 0; i < ETHER_CFG_EMAC_RX_DESCRIPTORS; i++)
    {
        ...
        p_descriptor = (descriptor_t *) &rx_descriptors[channel][i];
...
#else
        /* copy address */
        p_descriptor->p_buf = (uint8_t *) &(ether_buffers[channel].buffer[i][0]);
#endif
...
#if (ETHER_CFG_USE_MMU)
        /* calc address */
        R_MMU_VAtoPA((uint32_t) &rx_descriptors[channel][(i + 1)], (uint32_t*)&(p_descriptor->next));
#else
        p_descriptor->next = (descriptor_t *) &rx_descriptors[channel][(i + 1)];
#endif
    }
...
#if (ETHER_CFG_USE_MMU)
    /* calc address */
    R_MMU_VAtoPA((uint32_t) &rx_descriptors[channel][0], (uint32_t*)&(p_descriptor->next) );
#else
    p_descriptor->next = (descriptor_t *) &rx_descriptors[channel][0];
#endif
    ...
    for (i = 0; i < ETHER_CFG_EMAC_TX_DESCRIPTORS; i++)
    {
        ...
        p_descriptor = (descriptor_t *) &tx_descriptors[channel][i];

#if (ETHER_CFG_USE_MMU)
        /* calc address */
        R_MMU_VAtoPA((uint32_t)&(ether_buffers[channel].buffer[(ETHER_CFG_EMAC_RX_DESCRIPTORS + i)][0]),
...
#else
        /* copy address */
        p_descriptor->p_buf = (uint8_t *) &(ether_buffers[channel].buffer[(ETHER_CFG_EMAC_RX_DESCRIPTORS + i)][0]);
#endif
...
#if (ETHER_CFG_USE_MMU)
        /* calc address */
        R_MMU_VAtoPA((uint32_t) &(tx_descriptors[channel][(i + 1)]), (uint32_t*)&p_descriptor->next);
#else
        p_descriptor->next = (descriptor_t *) &(tx_descriptors[channel][(i + 1)]);
#endif
...
#if (ETHER_CFG_USE_MMU)
    /* calc address */
    R_MMU_VAtoPA((uint32_t) &tx_descriptors[channel][0], (uint32_t*)&p_descriptor->next);
#else
    p_descriptor->next = (descriptor_t *) &tx_descriptors[channel][0];
#endif
